...
PS_GBUFFER_OUT PixelShaderEntry(VSOut input)
{
    ...
    //process material
    output.ambi_out = input.ambient;
    output.diff_out = ProcessDiffuse(input.tex, diff_type, 0, 1, 2, input.diffuse, gamma);
    output.spec_out = ProcessSpecular(input.tex, spec_type, 3, input.specular);
    output.refl_out = input.reflect;
